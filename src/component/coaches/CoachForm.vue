<template>
    <form  @submit.prevent="submitForm">
        <div class="form-control" :class="{invalid: !firstName.isValid}">
            ...
            <p v-if="!firstName.isValid">First Name is Must Fill</p>
        </div>
        <div class="form-control" :class="{invalid: !lastName.isValid}">
            <label for="lastname">Lastname</label>
            <input type="text" id="lastname"  v-model.trim="lastName.val" @blur="clearValidity('lastName')">
            <p v-if="!lastName.isValid">Last Name is Must Fill</p>
        </div>
        <div class="form-control" :class="{invalid: !description.isValid}">
            <label for="rate">Description</label>
            <textarea type="text" id="rate" v-model.trim="description.val" rows="5" @blur="clearValidity('description')"></textarea>
            <p v-if="!description.isValid">Description is Must Fill</p>
        </div>
        <div class="form-control" :class="{invalid: !rate.isValid}">
            <label for="rate">Rate</label>
            <input type="number" id="rate" v-model.number="rate.val">
            <p v-if="!rate.isValid">Rate is Must Fill</p>
        </div>
        <div class="form-control" :class="{invalid: !areas.isValid}" @blur="clearValidity('areas')">
            ...
            <p v-if="!areas.isValid">Please Choose One</p>
        </div>
        <p v-if="!formIsValid">Please Check Your Value</p>
